    ...
    <div className="flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey max-lg:border-b">
      {
        Object.keys(stats).map((key, i) => {
          return !key.includes("parent") ? <div key={i} className={"flex flex-col items-center w-full h-full justify-center p-4 px-6 " + (i != 0 ? "border-grey border " : "")}> 

          <h1 className=" text-xl lg:text-2xl mb-2">{stats[key].toLocaleString()}</h1>

          <p className=" max-xl:text-dark-grey capitalize">{key.split("_")[1]}</p>
      ...
      <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
        ...
        <div className="flex flex-col justify-center py-2 w-full min-w-[300px]">
          <div>
            ...
            <p className="line-clamp-1">Publicshed on { getDay(publishedAt) }</p>
          </div>
...
export const ManageDraftBlogPost = ({ blog }) => {
  let { banner, blog_id, title, des, index, publishedAt, activity } = blog;
