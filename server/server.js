...
import serviceAccountKey from "./mernblogb-firebase-adminsdk-pkrqa-a6c50c136f.json" assert {type: "json"}
...
server.post("/signup", (req, res) => {
    ...
    bcrypt.hash(password, 10, async (err, hashed_password) => {
        ...
        let username = await generateUsername(email);  // if we give "as@gmail.com" -> so it split to [as, gamil], becouse we use [0] so it return us "as" 
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
        ...
        .then(async (decodedUser) => {
            ...
            if (user) { //login
                if (!user.google_auth) {
                    return res.status(403).json({ "error": "This email wan signed up without google. please lo in with password to access the account" })
                }
            }
            else {
        ...
        .catch(err => {
            return res.status(500).json({ "error": "Failed to authhenticate you with google, Tyry with some other google account" })
        })
    ...
    User.findOne({ _id: req.user })
        .then((user) => {
            ...
            bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
                if (err) {
                    return res.status(500).json({ error: "Some error occured while changing the password, please try again after sometime" })
                }
                ...
                bcrypt.hash(newPassword, 10, (err, hashed_password) => {
                    ...
                    User.findOneAndUpdate({ _id: req.user }, { "personal_info.password": hashed_password })
                        ...
                        .catch(err => {
                            return res.status(500).json({ error: 'Some error occured while saving new password, please try again after sometime' })
                        })
                })
            })
        })
...
server.post('/add-comment', verifyJWT, (req, res) => {
    ...
    new Comment(commentObj).save().then(async commentFile => {
        ...
        Blog.findOneAndUpdate({ _id }, { $push: { "comments": commentFile._id }, $inc: { "activity.total_comments": 1, "activity.total_parent_comments": replying_to ? 0 : 1 } })
            .then(blog => { console.log('New commet created') });
...
server.post("/get-blog-commets", (req, res) => {
